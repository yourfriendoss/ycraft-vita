...
void VideoComponents::Init() {
	if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO ) < 0) {
		Logs::Instance().Error("Failed to initialise SDL: %s\n", SDL_GetError());
	}

	if(SDL_InitSubSystem(SDL_INIT_JOYSTICK | SDL_INIT_GAMECONTROLLER) < 0) {
		Logs::Instance().Error("Failed to initialise SDL Controller API: %s\n", SDL_GetError());
	}
	...
	if (window == nullptr) {
		Logs::Instance().Error("Failed to create window: %s", SDL_GetError());
	}
	Logs::Instance().Log("Created window");
	...
	if (renderer == nullptr) {
		Logs::Instance().Error("Failed to create renderer: %s", SDL_GetError());
	}
	Logs::Instance().Log("Created renderer");
